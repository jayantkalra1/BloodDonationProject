...
def qr_dashboard_view(request):
    totalunit=models.Stock.objects.aggregate(Sum('unit'))
    dict={
        'A+': str(models.Stock.objects.get(bloodgroup="A+").unit),
        'A-': str(models.Stock.objects.get(bloodgroup="A-").unit),
        'B+': str(models.Stock.objects.get(bloodgroup="B+").unit),
        'B-': str(models.Stock.objects.get(bloodgroup="B-").unit),
        'AB+': str(models.Stock.objects.get(bloodgroup="AB+").unit),
        'AB-': str(models.Stock.objects.get(bloodgroup="AB-").unit),
        'O+':  str(models.Stock.objects.get(bloodgroup="O+").unit),
        'O-':  str(models.Stock.objects.get(bloodgroup="O-").unit)
    }

    dataJSON = dumps(dict)
   
    return render(request, 'blood/qrcode.html', {'data': dataJSON})